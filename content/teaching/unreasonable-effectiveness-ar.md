...
This is less surprising once one recognizes what $\theta$ and $y$ are doing. Pretraining deposits in $\theta$ a library of learned procedures — arithmetic, parsing, search heuristics, code patterns, domain intuition — and $p_\theta(X \mid y)$ is a Bayesian posterior that combines this rich prior with the evidence in $y$. A few tokens of context can therefore select which procedures fire and shift the output distribution dramatically.
...
Return to the qubit calibration experiment from the opening. The agent driving it is exactly this sampler wrapped in a harness. Code becomes an experimental action, the measured trace becomes new context, and the next sampled action updates the calibration — all in pure text. The agent reads raw numerical traces, writes fitting code, and reasons about the results; no vision model is involved. Multimodality, here, comes from tool use rather than from a fused vision encoder. A recent benchmark, QCalEval, makes the contrast concrete: it evaluates how well vision-language models (VLMs) interpret calibration *plots* — images of the same oscillation traces and spectroscopy maps that our agent handles as arrays.[^8] Current VLMs see visual features but struggle to diagnose them reliably. Rich in-context demonstrations close most of the gap on frontier models — itself an instance of context steering — while fine-tuning is an economic workaround to bring smaller open models toward the same ceiling. An agent with programmatic access to the instrument already has the data; routing it through a plotting pipeline and a vision encoder is a lossy detour.
...
[^1]: Xu, Han, Ou, Ye, Shen, Gao, Wang, Che, Song, Liu, Wang, Zhang, Zhang, and Yu, "Vibe Calibration: Autonomous Bring-up of a 112-Qubit Superconducting Quantum Processor by a Skill-Orchestrating Language Agent", arXiv:2606.22376 (2026). https://arxiv.org/abs/2606.22376

[^2]: Wu, Wang, and Zhang, "Solving Statistical Mechanics Using Variational Autoregressive Networks", Physical Review Letters 122, 080602 (2019). https://arxiv.org/abs/1809.10606

[^3]: Tian, Jiang, Yuan, Peng, and Wang, "Visual Autoregressive Modeling: Scalable Image Generation via Next-Scale Prediction", arXiv:2404.02905 (2024). https://arxiv.org/abs/2404.02905

[^4]: Han, Ghazvininejad, Koh, and Tsvetkov, "JPEG-LM: LLMs as Image Generators with Canonical Codec Representations", arXiv:2408.08459 (2024). https://arxiv.org/abs/2408.08459
...
[^6]: Cao, Ou, and Wang, "CrystalFormer-CSP: Thinking Fast and Slow for Crystal Structure Prediction", arXiv:2512.18251 (2025). https://arxiv.org/abs/2512.18251

[^7]: Yuksekgonul, Koceja, Li, Bianchi, McCaleb, Wang, Kautz, Choi, Zou, Guestrin, and Sun, "Learning to Discover at Test Time", arXiv:2601.16175 (2026). https://arxiv.org/abs/2601.16175

[^8]: Cao, Pancotti, Lubowe, Svore, Kyoseva, Stanwyck, Costa, Zhang, Mantilla Calderon, and Aspuru-Guzik, "QCalEval: Benchmarking Vision-Language Models for Quantum Calibration Plot Understanding" (NVIDIA, April 2026). https://research.nvidia.com/publication/2026-04_qcaleval-benchmarking-vision-language-models-quantum-calibration-plot
